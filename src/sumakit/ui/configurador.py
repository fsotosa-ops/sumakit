...
def _paleta_desde_estado(base: theme.Palette) -> theme.Palette:
    categorical = tuple(
        st.session_state.get(f"cat{i}", c) for i, c in enumerate(base.categorical)
    )
    otros = {campo: st.session_state.get(campo, getattr(base, campo))
...
def _figura_ejemplo(pal: theme.Palette):
    ...
    with theme.using(pal):
        ...
        for i, etiqueta in enumerate(["Serie A", "Serie B", "Serie C"]):
            ax1.plot(x, np.cumsum(rng.normal(1, 1, 12)), label=etiqueta,
...
def _lamina_ejemplo(pal: theme.Palette):
    ...
    ax.text(0.5, 1.05, "Dos tercios de la cartera realizó tres transacciones",
            fontsize=13, color=pal.text_primary)
    ax.text(0.5, 1.5, "o menos en el mes", fontsize=13, color=pal.categorical[0],
            fontweight="bold")
    ax.add_patch(plt.Rectangle((0.5, 1.9), 7.6, 4.7, facecolor=pal.grid, alpha=0.25))
    ax.add_patch(plt.Rectangle((8.6, 2.1), 4.2, 1.2, facecolor=pal.surface,
                               edgecolor=pal.categorical[0], linestyle="--", lw=1.4))
    ax.text(8.8, 2.6, "La anotación que dice\nqué mirar", fontsize=8.5,
...
def principal() -> None:
    ...
    st.sidebar.selectbox(
        "Tipografía", ["Helvetica Neue", "Inter", "DejaVu Sans", "Georgia"],
        key="fuente")
    ...
    st.sidebar.caption("El orden es el mecanismo de seguridad para daltonismo, "
    ...
    with der:
        ...
        st.caption("Se recalcula en cada cambio. Ninguna herramienta de diseño "
                   "revisa esto por ti.")
        modo = st.radio("Forma del gráfico", ["Barras y líneas", "Scatter y pairplot"],
                        horizontal=True,
        ...
        try:
            ...
            for _, fila in informe.iterrows():
                ...
                st.markdown(f"{icono} **{fila['chequeo']}** — {fila['valor']} "
                            f"(mínimo {fila['umbral']})")
        ...
        if st.button("Escribir el tema", type="primary"):
            ruta = Path(destino)
            ruta.write_text(json.dumps(_serializar(pal), indent=2, ensure_ascii=False),
                            encoding="utf-8")
            st.success(f"Escrito en {ruta.resolve()}")
            st.code("from sumakit import theme\n"
                    f'theme.use(theme.load("{ruta}"))', language="python")
